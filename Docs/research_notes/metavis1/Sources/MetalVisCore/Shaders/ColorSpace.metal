...
// MARK: - Test Kernels (Deprecated - Move to Tests/Shaders/)
// TODO: Remove these from production builds

#ifdef INCLUDE_TEST_KERNELS

kernel void convertSRGBToLinear(
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    float3 linear = Core::ColorSpace::SRGBToLinear(color.rgb);
    dest.write(float4(linear, color.a), gid);
}

kernel void convertLinearToSRGB(
    texture2d<float, access::read> source [[texture(0)]],
    texture2d<float, access::write> dest [[texture(1)]],
    uint2 gid [[thread_position_in_grid]])
{
    ...
    float4 color = source.read(gid);
    float3 srgb = Core::ColorSpace::LinearToSRGB(color.rgb);
    dest.write(float4(srgb, color.a), gid);
}

#endif // INCLUDE_TEST_KERNELS

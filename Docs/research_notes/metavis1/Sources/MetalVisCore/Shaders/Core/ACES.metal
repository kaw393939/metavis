...
// Using half precision for intermediate calculations
inline float3 ACES_saturation_sweetener(float3 acescg, float saturation) {
    // Use half-precision luma for performance (sufficient for saturation ops)
    half luma = dot(half3(acescg), Core::Color::LumaACEScg_h);
    half3 diff = half3(acescg) - half3(luma);
    return float3(half3(luma) + diff * half(saturation));
}

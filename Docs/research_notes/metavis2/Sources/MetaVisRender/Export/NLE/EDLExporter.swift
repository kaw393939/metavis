...
/// Exports timelines to EDL format
public struct EDLExporter {
    ...
    private func extractElementInfo(_ element: ManifestElement) -> (id: String, name: String, duration: Double) {
        ...
        case .model(let modelElement):
            let id = URL(fileURLWithPath: modelElement.path).deletingPathExtension().lastPathComponent
            let name = id
            return (id, name, 10.0) // Default duration for models
        }
    }

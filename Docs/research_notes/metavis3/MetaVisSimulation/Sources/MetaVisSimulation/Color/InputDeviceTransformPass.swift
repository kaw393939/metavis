...
public class InputDeviceTransformPass {
    ...
    public init(device: MTLDevice, library: MTLLibrary? = nil) throws {
        ...
        // Use provided library or load from bundle
        ...
        if let lib = library {
            ...
            throw ColorError.shaderLibraryNotFound
        }
        
        // Create IDT pipeline
        guard let idtFunction = shaderLibrary.makeFunction(name: "cs_idt_transform") else {
        ...
        // Create ODT pipeline
        guard let odtFunction = shaderLibrary.makeFunction(name: "cs_odt_transform") else {

...
namespace ColorSpace {
    ...
    enum TransferFn {
        ...
        TF_HLG       = 4, // BT.2100
        TF_APPLE_LOG = 5,
        TF_LOGC3     = 6, // ARRI LogC3
        TF_SLOG3     = 7  // Sony S-Log3
    };
    ...
    enum Primaries {
        ...
        PRIM_REC2020 = 4,
        PRIM_ALEXA   = 5, // ARRI Wide Gamut 3
        PRIM_SGAMUT3 = 6  // Sony S-Gamut3
    };
    ...
    // ARRI Alexa Wide Gamut 3 (D65) <-> XYZ (D65)
    constant float3x3 M_AlexaWide_to_XYZ = float3x3(
        float3(0.638008, 0.214704, 0.097744),
        float3(0.291954, 0.823841, -0.115795),
        float3(0.002798, -0.067034, 1.153294)
    );
    constant float3x3 M_XYZ_to_AlexaWide = float3x3(
        float3(2.00411699, -0.53896761, -0.22401091),
        float3(-0.70740668, 1.68505799, 0.21514831),
        float3(-0.04596131, 0.09108227, 0.96468692)
    );

    // Sony S-Gamut3 (D65) <-> XYZ (D65)
    constant float3x3 M_SGamut3_to_XYZ = float3x3(
        float3(0.5990839208, 0.2489255166, 0.1024464979),
        float3(0.2150758200, 0.8850685017, -0.1001443216),
        float3(0.0320658535, -0.0322058443, 1.0892842641)
    );
    constant float3x3 M_XYZ_to_SGamut3 = float3x3(
        float3(2.0707746765, -0.5922158264, -0.2491980866),
        float3(-0.5068794247, 1.4079548389, 0.1758345529),
        float3(-0.0759629153, 0.0578968988, 0.9261603695)
    );
    ...
    // ACEScg <-> ARRI Wide Gamut 3 (via D60/D65 CAT)
    constant float3x3 M_AlexaWide_to_ACEScg = M_XYZ_to_ACEScg * M_CAT_D65_to_D60 * M_AlexaWide_to_XYZ;
    constant float3x3 M_ACEScg_to_AlexaWide = M_XYZ_to_AlexaWide * M_CAT_D60_to_D65 * M_ACEScg_to_XYZ;
    
    // ACEScg <-> Sony S-Gamut3 (via D60/D65 CAT)
    constant float3x3 M_SGamut3_to_ACEScg = M_XYZ_to_ACEScg * M_CAT_D65_to_D60 * M_SGamut3_to_XYZ;
    constant float3x3 M_ACEScg_to_SGamut3 = M_XYZ_to_SGamut3 * M_CAT_D60_to_D65 * M_ACEScg_to_XYZ;
    ...
    // ARRI LogC3 (Linear -> LogC3)
    // Note: We usually need LogC3 -> Linear for IDT
    inline float3 LogC3ToLinear(float3 logc) {
        // Parameters for LogC3 (EI 800)
    ...
    // Sony S-Log3 (Log -> Linear)
    inline float3 SLog3ToLinear(float3 slog) {
        // S-Log3 parameters
        // y = (420 + log10((x + 0.01) / (0.18 + 0.01)) * 261.5) / 1023
        // Inverse: x = pow(10, ((y * 1023 - 420) / 261.5)) * (0.19) - 0.01
        
        float3 linear;
        for(int i=0; i<3; i++) {
            float y = slog[i];
            if (y >= 171.2102946929f / 1023.0f) {
                linear[i] = pow(10.0f, (y * 1023.0f - 420.0f) / 261.5f) * (0.18f + 0.01f) - 0.01f;
            } else {
                linear[i] = (y * 1023.0f - 95.0f) / 0.01125000f / (171.2102946929f - 95.0f) * (0.01f); // Linear segment approx
                // Actually, S-Log3 is pure log above a very low threshold.
                // The official formula usually covers the whole range or has a small linear toe.
                // Using the standard inverse:
                linear[i] = pow(10.0f, (y * 1023.0f - 420.0f) / 261.5f) * 0.19f - 0.01f;
            }
        }
        return linear;
    }
    ...
    // ACEScg <-> Sony S-Gamut3
    inline float3 SGamut3ToACEScg(float3 s)   { return applyMatrix(M_SGamut3_to_ACEScg, s); }
    inline float3 ACEScgToSGamut3(float3 acescg){ return applyMatrix(M_ACEScg_to_SGamut3, acescg); }
    
    // ACEScg <-> ARRI Wide Gamut 3
    inline float3 AlexaWideToACEScg(float3 a) { return applyMatrix(M_AlexaWide_to_ACEScg, a); }
    inline float3 ACEScgToAlexaWide(float3 acescg){ return applyMatrix(M_ACEScg_to_AlexaWide, acescg); }
    ...
    inline float3 DecodeToACEScg(float3 encodedRGB, int transferFn, int primaries) {
        ...
        switch (transferFn) {
            ...
            case TF_LOGC3:     srcLinear = LogC3ToLinear(encodedRGB); break;
            case TF_SLOG3:     srcLinear = SLog3ToLinear(encodedRGB); break;
            case TF_LINEAR:
        ...
        switch (primaries) {
            ...
            case PRIM_ALEXA:   acescg = AlexaWideToACEScg(srcLinear); break;
            case PRIM_SGAMUT3: acescg = SGamut3ToACEScg(srcLinear); break;

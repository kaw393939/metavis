...
- No first-class editing engine for full ripple/roll/slip/slide invariants (commands exist, but invariants are not comprehensively proven).
- Retime is represented as a clip effect (`mv.retime`) and is applied as video time-mapping.

### Video time mapping
- Video sources use clip-local time mapping (procedural + file-backed).
- `TimelineCompiler` is frame-based and only compiles video tracks; audio is rendered separately.
- Transition *types* (`dip`, `wipe`) are modeled but not implemented in compositing; only alpha-based fades/crossfades are currently used. (`dip`/`wipe` are out of scope for Sprint 13.)

### Audio pipeline
- File-backed audio playback is implemented (non-`ligm://` audio sources can decode and play).

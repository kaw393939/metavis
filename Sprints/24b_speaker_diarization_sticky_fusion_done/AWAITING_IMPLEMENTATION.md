...
> [!NOTE]
> **Status (2025-12-22):** This document is retained for history; the Sprint 24b contract items referenced here are now implemented.
...
- The current transcript JSONL record (`TranscriptWordV1`) does not carry `attributionConfidence`.
- Implemented: a separate, versioned attribution sidecar keyed by `wordId`:
	- `<out>/transcript.attribution.v1.jsonl`
	- schema: `transcript.attribution.v1`
	- record: `TranscriptAttributionV1` in `MetaVisCore`
- Optional future: bump the transcript word schema (e.g. `transcript.word.v2`) to add `attributionConfidence: ConfidenceRecordV1` for a single-file contract.
...
Implemented: `identity.timeline.v1.json` containing:

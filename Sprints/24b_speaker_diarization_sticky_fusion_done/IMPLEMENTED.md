...
- **Governed confidence surfaces** at the word level using the shared `MetaVisCore` confidence record (`ConfidenceRecordV1`) with discrete grades + finite reason codes (`ReasonCodeV1`).
	- Implemented: a dedicated sidecar keyed by `wordId` (`transcript.attribution.v1.jsonl`) carrying `ConfidenceRecordV1`.
	- Optional future: schema bump (e.g. `transcript.word.v2`) if/when consumers prefer a single-file contract.
...
- `<out>/transcript.attribution.v1.jsonl` (wordId-keyed attribution confidence using `ConfidenceRecordV1`)
- `<out>/captions.vtt` (WebVTT with `<v Speaker>` voice tags)
- `<out>/speaker_map.v1.json` (stable mapping of speakerId → speakerLabel)

Implemented internal artifact:
- `<out>/identity.timeline.v1.json` (cluster lifecycle fields, spans including `OFFSCREEN`, and bindings)

Notes:
- The identity timeline includes deterministic cluster lifecycle fields (`bornAtSeconds`, `lastActiveAtSeconds`, `frozen`, `mergeCandidates`) and carries bindings evidence from `identity.bindings.v1`.
...
- Attribution sidecar contract: `Sources/MetaVisCore/Transcript/TranscriptAttributionV1.swift`
